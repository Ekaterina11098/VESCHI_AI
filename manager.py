...
# Простая база данных бенчмарков (в памяти сервера)
if "benchmarks" not in st.session_state:
    st.session_state["benchmarks"] = []

def save_to_benchmarks(feedback_data, original_draft, final_answer):
    """Сохраняет успешно опубликованный ответ в базу лучших практик"""
...
# Извлекаем отзывы из памяти приложения
...
if not feedbacks_list:
    ...
    for idx, fb in enumerate(feedbacks_list):
        ...
        text = fb.get("text", "⚠️ Отзыв без текста (оценка без комментария)")
        product_name = fb.get("productName", "Товар бренда VESCHI")
        
        # Красивая карточка каждого отзыва
        with st.container():
            ...
            # Кнопка ИИ-генерации вариантов
            btn_key = f"gen_{fb_id}_{idx}"
            if st.button("✨ Создать черновики AI (2 варианты)", key=btn_key):
                with st.spinner("🤖 Нейросеть VESCHI AI анализирует отзыв и подбирает идеальные слова..."):
                    res = generate_draft(fb)
                    
                    # 🛠 РАСКЛЕИВАЕМ ТЕКСТ ИЗ JSON В ОБЫЧНЫЕ ВАРИАНТЫ
            ...
            # Показываем варианты, если они уже сгенерированы
            ...
            if v1_saved or v2_saved:
                ...
                with tab1:
                    ...
                    # Запуск жесткого валидатора бренда
                    val_res1 = validate_answer(edited_v1, fb)
                    if val_res1["approved"]:
                        st.success("✅ Черновик одобрен Валидатором! Все правила бренда VESCHI строго соблюдены.")
                        if val_res1["warnings"]:
                            for w in val_res1["warnings"]: st.warning(f"⚠️ Предупреждение: {w}")
                ...
                with tab2:
                    ...
                    if val_res2["approved"]:
                        st.success("✅ Черновик одобрен Валидатором! Все правила бренда VESCHI строго соблюдены.")
                        if val_res2["warnings"]:
                            for w in val_res2["warnings"]: st.warning(f"⚠️ Предупреждение: {w}")
                        ...
                        st.error("❌ Черновик заблокирован Validator!")
                        for err in val_res2["errors"]:
                            st.markdown(f"🔴 *{err}*")
        st.write("---")

# Вкладка успешных кейсов для аналитики в самом низу

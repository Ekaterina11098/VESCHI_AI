...
def get_wb_sales_speed(article):
    """📈 ИМИТАЦИЯ СКОРОСТИ ПРОДАЖ: Базовые 2 шт/день для теста аналитики лимитов."""
    return 2.0
...
async def check_cross_stocks(message: types.Message):
    ...
    for art, ms_stock in ms_results:
        ...
        if total_wb == 0 and ms_stock > 0:
            ...
            if safe_add > 0:
                report_lines.append(f"⚠️ **ДЕФИЦИТ НА 7 ДНЕЙ! Арт: `{art}`**\n• На WB хватит на **{round(days_left, 1)} дн.** | Свободно: {ms_stock} шт. | Догрузить: **+{safe_add} шт.**\n")
                issues_found += 1
...
async def check_tnved_and_rating_audit(message: types.Message):
    ...
    for cab_name, token in cabinets:
        ...
        for card in cards:
            ...
            # Умное извлечение характеристик для поиска скрытых дефектов
            characteristics = card.get("characteristics", [])
            description = str(card.get("description", "")).strip()
            media_files = card.get("mediaFiles", [])
            
            # 🛠 РАСЧЕТ ИСТИННОГО РЕЙТИНГА КОНТЕНТА:
            # Если WB выдает 10/10 по умолчанию, мы сами снизим балл, если карточка пустая!
            ...
            if not any("видео" in str(m).lower() for m in media_files) and len(media_files) > 0:
                computed_score = min(computed_score, 8.0)
                custom_reasons.append("Отсутствует видеообзор товара в галерее карточки")
            
            # Сверяем категорию товара с базой деклараций ТН ВЭД
            ref_row = None
            for key in REF_DATA:
            ...
            # 🛠 УМНАЯ СВЕРКА ТН ВЭД: проверяем вхождение первых 4 цифр декларации (например, 4202 или 6601)
            if ref_row:
            ...
            # Если нашли дефекты контента или низкий балл
